  ...
  return (
    <>
      <Grid xs={12} md={8} sx={{ p: 1 }}>
        <Box sx={{ minHeight: { xs: 0, md: 60 } }}>{Description}</Box>
      </Grid>
      <Grid
        xs={12}
        md={4}
        sx={{ p: 1, pl: 2, display: "flex", alignItems: "center" }}
      >
        {Button}
      </Grid>
    </>
      ...
      Button={
        <>
          <StyledButton
            ...
            fullwidth
            component={Link}
            to={`/${election.election_id}/admin/roles`}
          >
...
const ViewBallotSection = ({
  ...
  return (
    <Section
      Description={
        <>
          <Typography variant="h5">View ballots</Typography>
      ...
      Button={
        <>
          <StyledButton
            ...
            fullwidth
            component={Link}
            to={`/${election.election_id}/admin/ballots`}
          >
            View ballots
          </StyledButton>
        </>
        ...
        <>
          ...
          <Typography variant="body1" sx={{ pl: 2 }}>
            {election.state === "draft"
              ...
              : "View the status of your voters"}
          </Typography>
      ...
      Button={
        <>
          <StyledButton
            ...
            fullwidth
            component={Link}
            to={`/${election.election_id}/admin/voters`}
          >
            <Typography align="center" variant="body2">
...
const PreviewBallotSection = ({
  ...
  return (
    <Section
      Description={
        <>
          <Typography variant="h5">Preview the ballot</Typography>
          {!hasPermission(permissions, "canViewElectionRoll") && (
            <Typography variant="body1" sx={{ color: "error.main", pl: 2 }}>
              You do not have the correct permissions for this action
            </Typography>
...
const DuplicateElectionSection = ({
  permissions,
  duplicateElection,
}: {
  election: Election;
  permissions: string[];
  duplicateElection: () => void;
}) => {
  return (
    <Section
      Description={
        <>
          <Typography variant="h5">Duplicate</Typography>
          <Typography variant="body1" sx={{ pl: 2 }}>
    ...
    <Section
      Description={
        <>
          ...
          {!(
            hasPermission(permissions, "canViewPreliminaryResults") ||
      ...
      Button={
        <>
          <StyledButton
            ...
            fullwidth
            component={Link}
            to={`/${election.election_id}/results`}
          >
            View results
          </StyledButton>
        </>
...
const ArchiveElectionSection = ({
  permissions,
  archiveElection,
}: {
  election: Election;
  permissions: string[];
  archiveElection: () => void;
}) => {
  return (
    <Section
      Description={
        <>
          ...
          <Typography variant="body1" sx={{ pl: 2 }}>
            Achives election, preventing future changes and hiding it from on
            the elections page
          </Typography>
          {!hasPermission(permissions, "canEditElectionState") && (
            <Typography variant="body1" sx={{ color: "error.main", pl: 2 }}>
...
const ShareSection = ({
  ...
  return (
    <Section
      Description={
        <>
          <Typography variant="h5">Share your election</Typography>
        </>
      }
      Button={
        <>
          <ShareButton
            url={`${window.location.origin}/Election/${election.election_id}`}
          />
        </>
      }
    />
...
const AdminHome = () => {
  ...
  const finalizeElection = async () => {
    ...
    const confirmed = await confirm({
      ...
      message:
        "Are you sure you want to finalize your election? Once finalized you won't be able to edit it.",
    });
    if (!confirmed) return;
    try {
      await finalize();
  ...
  const archiveElection = async () => {
    ...
    const confirmed = await confirm({
      ...
      message:
        "Are you sure you wish to archive this election? This action cannot be undone.",
    });
    if (!confirmed) return;
        ...
        {election.state !== "draft" && election.state !== "finalized" && (
          <>
            ...
            <TogglePublicResultsSection
              election={election}
              permissions={permissions}
              togglePublicResults={togglePublicResults}
            />
            <Divider style={{ width: "100%" }} />
        ...
        {election.state === "draft" && (
          <Grid xs={12} sx={{ p: 1 }}>
            ...
            <FinalizeSection
              election={election}
              permissions={permissions}
              finalizeElection={finalizeElection}
            />
          </Grid>
        )}
      </Grid>
