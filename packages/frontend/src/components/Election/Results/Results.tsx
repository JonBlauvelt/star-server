...
const GenericDetailedStepsWidget = ({
  results,
  rounds,
}: {
  title: string;
  results: approvalResults | rankedRobinResults;
  rounds: number;
}) => {
  return (
    <div className="detailedSteps">
      {results.roundResults.map((round, r) => (
        <>
          {rounds > 1 && (
            <Typography variant="h4">{`Winner ${r + 1}`}</Typography>
          )}
          <ol>
            {round.logs.map((log) => (
              <li key={log}>{log}</li>
...
function STARResultViewer({
  ...
  const noPrefStarData = results.summaryData.noPreferenceStars.map(
    (count, i) => ({
      name: `${i}⭐`,
      count: count,
    }),
  ...
  return (
    <>
      <WinnerResultTabs numWinners={rounds}>
        {roundIndexes.map((i) => (
          <STARResultSummaryWidget key={i} results={results} roundIndex={i} />
        ))}
      </WinnerResultTabs>
    ...
    <div className="resultViewer">
      ...
      <Paper elevation={0} sx={{ width: "100%" }}>
        <TableContainer
          sx={{
            maxHeight: 600,
            maxWidth: { xs: 300, sm: 500, md: 600, lg: 1000 },
          }}
        >
          <Table size="small" stickyHeader aria-label="sticky table">
            ...
            <TableBody>
              ...
              <TableRow>
                ...
                {results.exhaustedVoteCounts.map((exhaustedVoteCount, i) => (
                  <TableCell key={i} align="right">
                    {exhaustedVoteCount}
                  </TableCell>
                ))}
          ...
          <Table size="small">
            <TableHead>
              <TableRow>
                ...
                <TableCell className="matrix">
                  {" "}
                  # of wins head to head wins
                </TableCell>
              </TableRow>
            </TableHead>
            <TableBody>
              {results.summaryData.candidates.map((c, n) => (
                <TableRow key={`${c.index}-${c.name}`}>
                  ...
                  <TableCell>
                    {" "}
                    {results.summaryData.totalScores[n].score}{" "}
                  </TableCell>
                </TableRow>
              ))}
            </TableBody>
    ...
    <>
      ...
      <DetailExpander title="Details">
        <DetailExpanderGroup defaultSelectedIndex={-1}>
          <GenericDetailedStepsWidget
            title="Detailed Steps"
            results={results}
            rounds={rounds}
          />
          <div
            title="How Approval Voting works"
          ...
          >
            <iframe
              ...
              src="https://www.youtube.com/embed/db6Syys2fmE"
...
function ResultViewer({
  ...
  votingMethod: string;
  results: irvResults | pluralityResults | rankedRobinResults;
  children: any;
  learnLink?: string | undefined;
}) {
  return (
    <Box className="resultViewer">
      ...
      >
        ...
        {learnLink && (
          <>
            <br />
            <a href={learnLink} style={{ color: "inherit" }}>
              How {votingMethod} Works
            </a>
          </>
        )}
      </Typography>
        ...
        <tbody className="matrix">
          ...
          {result.summaryData.weightedScoresByRound[0].map((col, cand_ind) => (
            <tr className="matrix" key={`d${cand_ind}`}>
              <th className="matrix" key={`dh${cand_ind}`}>
                {result.summaryData.candidates[cand_ind].name}
              </th>
              {result.summaryData.weightedScoresByRound.map(
                (row, round_ind) => {
                  const score =
                    Math.round(
                      result.summaryData.weightedScoresByRound[round_ind][
                        cand_ind
                      ] * 10,
                    ) / 10;
                  return result.elected[round_ind].index ===
                    result.summaryData.candidates[cand_ind].index ? (
...
export default function Results({ race, result }: ResultsProps) {
  const { election } = useElection();
  let showTitleAsTie = ["random", "5-star"].includes(
    result.results.tieBreakType,
  );
  const removeTieBreakFromTitle =
    election.settings.break_ties_randomly &&
    result.results.tieBreakType == "random";
  ...
  if (!showTitleAsTie && result.votingMethod === "STAR") {
    ...
    const showTieBreakerWarning = (
      result.results as starResults
    ).roundResults.some((round) =>
      round.logs.some((log) => log.includes("tiebreaker")),
    );
    ...
    <div>
      <div className="flexContainer" style={{ textAlign: "center" }}>
        <Box sx={{ pageBreakAfter: "avoid", pageBreakInside: "avoid" }}>
          {result.results.summaryData.nValidVotes == 0 && (
            <h2>
          ...
          {result.results.summaryData.nValidVotes >= 1 && (
            <>
              {showTitleAsTie ? (
                <>
                  ...
                  {!removeTieBreakFromTitle && (
                    <Typography component="p" sx={{ fontWeight: "bold" }}>
                      (
                      {formatter.format(
                        result.results.elected.map((c) => c.name),
                      )}{" "}
                      won after tie breaker)
                    </Typography>
                  )}
                </>
              ) : (
                <Typography variant="h5" sx={{ fontWeight: "bold" }}>
                  ⭐{" "}
                  {formatter.format(result.results.elected.map((c) => c.name))}{" "}
                  Wins! ⭐
                </Typography>
              )}
              <Typography variant="h6">
                {result.results.summaryData.nValidVotes} voters
              </Typography>
            </>
          )}
        </Box>
        {result.results.summaryData.nValidVotes == 1 && (
          <p>
            There&apoas;s only one vote so far.
            <br />
            Full results will be displayed once there&apoas;s more votes.
          </p>
        )}
        {result.results.summaryData.nValidVotes > 1 && (
          <>
            {result.votingMethod === "STAR" && (
              <ResultViewer
                votingMethod="Star Voting"
                results={result.results}
                learnLink="https://www.youtube.com/watch?v=3-mOeUXAkV0"
              >
                <STARResultViewer
                  results={result.results}
                  rounds={race.num_winners}
                />
              </ResultViewer>
            )}

            {result.votingMethod === "Approval" && (
              <ResultViewer
                votingMethod="Approval Voting"
                results={result.results}
              >
                <ApprovalResultsViewer
                  results={result.results}
                  rounds={race.num_winners}
                />
              </ResultViewer>
