...
export default function STARDetailedResults({
  results,
}: {
  results: starResults;
  rounds: number;
  ...
  const tableData: candidateTableEntry[] = results.summaryData.candidates.map(
    (c, i) => ({
      name: c.name,
            ...
            <BarChart data={runoffData} barCategoryGap={5} layout="vertical">
              ...
              >
                {runoffData.map((entry, index) => (
                  <Cell
                    key={`cell-${index}`}
                    fill={RUNOFF_COLORS[index % RUNOFF_COLORS.length]}
                  />
                ))}
              </Bar>
            </BarChart>
        ...
        <Widget title="Runoff Table">
          {makeResultTable("starRunoffTable", [
            ...
            ...runoffData.map((c, i) => [
              ...
              i == 2
                ? ""
                : `${Math.round((c.runoffVotes * 1000) / finalistVotes) / 10}%`,
            ]),
