...
interface EnhancedTableHeadProps {
  ...
  headCells: HeadCell[];
  filters: any[];
  setFilters: React.Dispatch<React.SetStateAction<(FilterGroups | string)[]>>;
}
...
const formatTableData = (headKeys, data) => {
  const fData = data.map((item) => {
    ...
    headKeys.forEach((key) => {
      fItem[key] =
        key in headCellPool
          ? headCellPool[key].formatter(item[key], item)
          : item[key];
    });
    fItem["raw"] = item;
    return fItem;
  ...
  return array.filter((row) => {
    return headCells.every((col, colInd) => {
      ...
      if (col.filterType === "search") {
        ...
        return row[col.id]
          .toLowerCase()
          .includes(filters[colInd].toLowerCase());
      }
          ...
          >
            ...
            {headCell.filterType === "search" && (
              <TextField
                ...
                onChange={(e) =>
                  handleSearchFilterChange(cellInd, e.target.value)
                }
                placeholder="Search"
              />
            )}
            {headCell.filterType === "groups" && (
              <FormControl sx={{ my: 1, width: 120 }} size="small">
                <Select
                  multiple
                  value={Object.keys(props.filters[cellInd]).filter(
                    (group) => props.filters[cellInd][group],
                  )}
...
export default function EnhancedTable(props: EnhancedTableProps) {
  ...
  const headCells: HeadCell[] = props.headKeys.map(
    (key) => headCellPool[key] as HeadCell,
  );
  ...
  const filteredRows = React.useMemo(() => {
    setPage(0);
    return filterData(
      formatTableData(props.headKeys, props.data),
      headCells,
      filters,
    );
  }, [filters, props.data]);
