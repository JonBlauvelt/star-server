...
export default function RaceForm({
  ...
  const onEditCandidate = (candidate: Candidate, index) => {
    ...
    applyRaceUpdate((race) => {
      ...
      while (
        candidates.length >= 2 &&
        candidates[candidates.length - 1].candidate_name == "" &&
        candidates[candidates.length - 2].candidate_name == ""
      ) {
        candidates.pop();
      }
    });
  ...
  const moveCandidate = (fromIndex: number, toIndex: number) => {
    applyRaceUpdate((race) => {
      const candidate = race.candidates.splice(fromIndex, 1)[0];
  ...
  return (
    <>
        ...
        <Grid item xs={12} sx={{ m: 0, p: 1 }}>
          <TextField
            ...
            onChange={(e) => {
              ...
              applyRaceUpdate((race) => {
                race.description = e.target.value;
              });
            }}
          />
          <FormHelperText error sx={{ pl: 1, pt: 0 }}>
          ...
          election.settings.voter_access !== "open" && (
            <Grid item xs={12}>
              <TextField
                ...
                type="text"
                value={
                  editedRace.precincts ? editedRace.precincts.join("\n") : ""
                }
                sx={{
                  m: 1,
                  boxShadow: 2,
          ...
          <Grid item xs={12} sx={{ m: 0, p: 1 }}>
            ...
            <TextField
              ...
              onChange={(e) => {
                ...
                applyRaceUpdate((race) => {
                  race.num_winners = parseInt(e.target.value);
                });
              }}
            />
            <FormHelperText error sx={{ pl: 1, pt: 0 }}>
        ...
        <Grid item xs={12} sx={{ m: 0, my: 1, p: 1 }}>
          <FormControl component="fieldset" variant="standard">
            ...
            <RadioGroup
              ...
              onChange={(e) =>
                applyRaceUpdate((race) => {
                  race.voting_method = e.target.value;
                })
              }
            >
              <FormControlLabel
              ...
              {flags.isSet("METHOD_RANKED_ROBIN") && (
                <>
                  ...
                  />
                  <FormHelperText sx={{ pl: 4, mt: -1 }}>
                    Rank candidates in order of preference, single winner or
                    multi-winner
                  </FormHelperText>
                </>
              )}
              ...
              {flags.isSet("METHOD_APPROVAL") && (
                <>
                  ...
                  />
                  <FormHelperText sx={{ pl: 4, mt: -1 }}>
                    Mark all candidates you approve of, single winner or
                    multi-winner
                  </FormHelperText>
                </>
              )}
              ...
              {showsAllMethods && (
                ...
                >
                  <FormHelperText>
                    These voting methods do not guarantee every voter an equally
                    powerful vote if there are more than two candidates.
                  </FormHelperText>
                </Box>
              )}
              {showsAllMethods && (
                <>
                  ...
                  />
                  <FormHelperText sx={{ pl: 4, mt: -1 }}>
                    Mark one candidate only. Not recommended with more than 2
                    candidates.
                  </FormHelperText>
                  ...
                  {flags.isSet("METHOD_RANKED_CHOICE") && (
                    <>
                      ...
                      />
                      <FormHelperText sx={{ pl: 4, mt: -1 }}>
                        Rank candidates in order of preference, single winner,
                        only recommended for educational purposes
                      </FormHelperText>
                    </>
                  )}
                </>
      ...
      <Stack spacing={2}>
        ...
        <AddCandidate onAddNewCandidate={onAddNewCandidate} />
      </Stack>
    </>
  );
}
