...
export default function ElectionDetailsForm({
  editedElection,
  applyUpdate,
  errors,
  setErrors,
}) {
  const timeZone = editedElection.settings.time_zone
    ? editedElection.settings.time_zone
    : DateTime.now().zone.name;

  const [enableStartEndTime, setEnableStartEndTime] = useState(
    isValidDate(editedElection.start_time) ||
      isValidDate(editedElection.end_time),
  );
  const [defaultStartTime, setDefaultStartTime] = useState(
    isValidDate(editedElection.start_time)
      ? editedElection.start_time
      : DateTime.now().setZone(timeZone, { keepLocalTime: true }).toJSDate(),
  );
  const [defaultEndTime, setDefaultEndTime] = useState(
    isValidDate(editedElection.end_time)
      ? editedElection.end_time
      : DateTime.now()
          .plus({ days: 1 })
    ...
    <Grid container>
      ...
      <Grid item xs={12} sx={{ m: 0, p: 1 }}>
        <TextField
          ...
          onChange={(e) => {
            ...
            applyUpdate((election) => {
              election.description = e.target.value;
            });
          }}
        />
        <FormHelperText error sx={{ pl: 1, pt: 0 }}>
      ...
      {enableStartEndTime && (
        <>
          <Grid item xs={4} sx={{ m: 0, p: 1 }} justifyContent="center">
            <FormControl fullWidth>
              ...
              <Select
                ...
                onChange={(e) =>
                  applyUpdate((election) => {
                    election.end_time = e.target.value;
                  })
                }
              >
                <MenuItem value={DateTime.now().zone.name}>
                  {DateTime.now().zone.name}
                </MenuItem>
                <Divider />
                {timeZones.map((tz) => (
              ...
              <Input
                ...
                onChange={(e) => {
                  ...
                  if (e.target.value == null || e.target.value == "") {
                    ...
                    applyUpdate((election) => {
                      election.start_time = DateTime.fromISO(e.target.value)
                        .setZone(timeZone, { keepLocalTime: true })
                        .toJSDate();
                    });
                    setDefaultStartTime(
          ...
          <Grid item xs={6} sx={{ m: 0, p: 1 }} justifyContent="center">
            <FormControl fullWidth>
              <InputLabel shrink>Stop Date</InputLabel>
              <Input
                type="datetime-local"
                ...
                onChange={(e) => {
                  ...
                  if (e.target.value == null || e.target.value == "") {
                    ...
                    applyUpdate((election) => {
                      election.end_time = DateTime.fromISO(e.target.value)
                        .setZone(timeZone, { keepLocalTime: true })
                        .toJSDate();
                    });
                    setDefaultEndTime(
