...
export default function ElectionDetails({
  ...
  const timeZone = election.settings.time_zone
    ? election.settings.time_zone
    : DateTime.now().zone.name;

  const [enableStartEndTime, setEnableStartEndTime] = useState(
    isValidDate(election.start_time) || isValidDate(election.end_time),
  );
  const [defaultStartTime, setDefaultStartTime] = useState(
    isValidDate(election.start_time)
      ? election.start_time
      : DateTime.now().setZone(timeZone, { keepLocalTime: true }).toJSDate(),
  );
  const [defaultEndTime, setDefaultEndTime] = useState(
    isValidDate(election.end_time)
      ? election.end_time
      : DateTime.now()
          .plus({ days: 1 })
          ...
          <Grid item xs={4} sx={{ m: 0, p: 1 }} justifyContent="center">
            <FormControl fullWidth>
              ...
              <Select
                ...
                onChange={(e) =>
                  applyElectionUpdate((election) => {
                    election.settings.time_zone = e.target.value;
                  })
                }
              >
              ...
              <Input
                ...
                onChange={(e) => {
                  ...
                  if (e.target.value == null || e.target.value == "") {
                    applyElectionUpdate(
                      (election) => (election.start_time = undefined),
                    );
                  } else {
                    applyElectionUpdate(
                      (election) =>
