...
const CreateElectionTemplates = () => {
  ...
  const [quickPoll, setQuickPoll] = useLocalStorage<NewElection>(
    "QuickPoll",
    null,
  );
  const cardColor =
  ...
  const onAddElection = async (updateFunc: (election: NewElection) => any) => {
    ...
    if (quickPoll) {
      ...
      const candidates = quickPoll.races[0].candidates.filter(
        (candidate) => candidate.candidate_name.length > 0,
      );
  ...
  return (
    <>
      {!authSession.isLoggedIn() && (
        <div> Must be logged in to create elections </div>
      )}
      {authSession.isLoggedIn() && (
        <Box
          display="flex"
          ...
          <Grid container spacing={3} maxWidth={1000}>
            ...
            <Grid item xs={12} sm={6} md={3}>
              <Card sx={{ backgroundColor: cardColor, minHeight: cardHeight }}>
                <CardActionArea
                  disabled={
                    !sharedConfig.ACCOUNT_CUSTOM_REGISTRATION_OVERRIDES.includes(
                      authSession.getIdField("sub"),
                    )
                  }
                  onClick={() =>
                    onAddElection((election) => {
              ...
              <Card sx={{ backgroundColor: cardColor, minHeight: cardHeight }}>
                ...
                >
                  <CardContent>
                    ...
                    <Typography variant="body2" color="text.secondary">
                      Voters receive unique email invitations, no log in
                      required
                    </Typography>
                  </CardContent>
                </CardActionArea>
              ...
              <Card sx={{ backgroundColor: cardColor, minHeight: cardHeight }}>
                ...
                >
                  <CardContent>
                    ...
                    <Typography variant="body2" color="text.secondary">
                      Voters receive email invitations but must create star.vote
              ...
              <Card sx={{ backgroundColor: cardColor, minHeight: cardHeight }}>
                ...
                >
                  <CardContent>
                    ...
                    <Typography variant="body2" color="text.secondary">
                      Election provide list of valid voter IDs and distribute
                      them to voters
                    </Typography>
                  </CardContent>
                </CardActionArea>
            ...
            <Grid item xs={12}>
              <Typography component="p">
                * Accounts must be approved first to create elections with
                custom registration
              </Typography>
              <Typography component="p">
                ** Free Tier elections are limited to{" "}
                {sharedConfig.FREE_TIER_PRIVATE_VOTER_LIMIT} voters
              </Typography>
              <Typography component="p">
                Email us at elections@equal.vote to be upgraded
              </Typography>
            </Grid>
          </Grid>
        </Box>
